...
export default function Home() {
  ...
  const toggleMute = async () => {
    if (isMuted) {
      // Only try to start VAD when unmuting
      try {
        await vad.start();
        setIsMuted(false);
      } catch (err) {
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMute}
        className="fixed bottom-4 right-4 p-4 rounded-full bg-neutral-200/80 dark:bg-neutral-800/80 text-neutral-700 hover:text-black dark:text-neutral-300 dark:hover:text-white border border-transparent hover:border-neutral-300 focus:border-neutral-400"
        aria-label={isMuted ? "Unmute microphone" : "Mute microphone"}
      >
        {isMuted ? (
          <MicOffIcon />
        ) : (
          <MicIcon className="text-red-500 drop-shadow-[0_0_10px_rgba(255,0,0,0.8)] animate-pulse" />
        )}
      </button>
        ...
        {messages.length === 0 && (
          <>
            ...
            {vad.loading ? (
              ...
              <p>Microphone is muted. Click the mic button to unmute.</p>
            ) : (
